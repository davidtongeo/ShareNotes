...
public class PostController {
    ...
    // we need the repository to get the raw users, its bad usage of the MVC tho.
    ...
    private ResponseEntity<Object> uploadComment(@RequestBody commentParam body) {
        try {
            var result = commentService.sendComment(
                    userRepository.findById(body.getUserId()).get(),
                    noteRepository.findById(body.getNoteId()).get(),
                    body.getContent());
